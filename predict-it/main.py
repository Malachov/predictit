...
if isinstance(config.predicted_columns_names, list):
    predicted_column_name = config.predicted_columns_names[0]
else:
...
# sklearn / neregrsivne mode curve fit

# Vice transformaci a do configu zvlast power transform, kter8 uhladi maly rozdily a zvetsi velke
# TODO numba
'''
...
** Další možnost, přímo definovat formáty dat**
@jit(int32(int32, int32))
def function(a, b):
'''

# TODO dask na vše - dask ml na tensorflow a sklearn
'''
...
lr.fit(train, test)
'''
# TODO / pca na vicerozmerna data

# TODO ujistit se o form8tu np.float64

# TODO  u sklearn test_train a crossvalidaci a boost

# TODO volitelny parametr smooth data - zaoblit data a tim predchazet overfittingu

# TODO confidence interval.py pro vykresleni a graf v plotly

# TODO - jako parametr pridani konstanty u makesequence

# TODO - zprovoznit pro vicerozmerna data - automatika, aby se nemusely vypinat modely pro jeden rozmer

# TODO do batch lstm.predict_sequencies_multiple(model, x_test, 50, 50)

# TODO dodelat plovouci prumer a plovouci smerodatnou odchylku do analyze, taky funkci pro korelační matici pokud máme externi vstupy

# TODO u tf batch, do funkce fit batch_size=predicts test
# TODO u tf zjistit rozdil vykonu v input_shape=(x, ) a input_dim=x
...
#def make_predictions(predicted_column_name):
    ...
    for j in range(results_shape[1]):
        for k in range(results_shape[2]):
            ...
            if config.criterion == 'mape':
                evaluated_matrix[i, j, k] = test_pre(results_matrix[i, j, k, :], test_matrix[i, j, k, :], criterion='mape')
...
if config.compareit:
    ...
    # Evaluate models for comparison
        ...
        try:
            ...
            if config.data_transform == 'difference':
                next_models_predicts[i] = inverse_difference(next_models_predicts[i], last_undiff_value)
