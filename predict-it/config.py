...
date_index = 0
predicts = 30  # Počet predikovaných hodnot - defaultně 7

datalength = 3000  # Posledních N prvků, které budou použity
...
confidence = 0.6  # Oblast nejistoty ve finálním grafu - vyšší hodnota znamená užší oblast - maximum 1
...
evaluate_test_data = 0  # Jestli bude model hodnocen podle testovacích dat a nebo pouze predikovaných dat
...
piclkeit = 1  # Uloží testovací data na disk v serializované formě, čímž zrychlí načítání - nutno vypnout na nulu, aby se data nenačítala pokaždé
...
models_parameters_limits = { 
       ...
       # "ARIMA (Autoregression integrated moving average)": {"p": [1, maxorder], "d": [0,1], "q": order, 'method': ['css-mle', 'mle', 'css'], 'trend': ['c', 'nc'], 'solver': ['lbfgs', 'bfgs', 'newton', 'nm', 'cg'], 'forecast_type': ['in_sample', 'out_of_sample']},
        ...
        "LSTM": {"n_steps_in": steps, "epochs": epochs, "units": units, "optimizer": models.lstm_optimizers, "loss": models.loses, "activation": models.activations, "timedistributed": [True, False], "metrics": [['mape'], ['mae'], ['mse'], ['cosine']]},
        "LSTM": {"n_steps_in": steps, "epochs": epochs, "units": units, "optimizer": models.lstm_optimizers, "loss": models.loses, "activation": models.activations, "timedistributed": [True, False], "metrics": [['mape'], ['mae'], ['mse'], ['cosine']]},
        "LSTM": {"n_steps_in": steps, "epochs": epochs, "units": units, "optimizer": models.lstm_optimizers, "loss": models.loses, "activation": models.activations, "timedistributed": [True, False], "metrics": [['mape'], ['mae'], ['mse'], ['cosine']]},
        "LSTM batch": {"n_steps_in": steps, "epochs": epochs, "units": units, "optimizer": models.lstm_optimizers, "loss": models.loses, "activation": models.activations},
        "LSTM": {"n_steps_in": steps, "epochs": epochs, "units": units, "optimizer": models.lstm_optimizers, "loss": models.loses, "activation": models.activations, "timedistributed": [True, False], "metrics": [['mape'], ['mae'], ['mse'], ['cosine']]},
        "LSTM": {"n_steps_in": steps, "epochs": epochs, "units": units, "optimizer": models.lstm_optimizers, "loss": models.loses, "activation": models.activations, "timedistributed": [True, False], "metrics": [['mape'], ['mae'], ['mse'], ['cosine']]},
        "Bidirectional LSTM": {"n_steps_in": steps, "epochs": epochs, "units": units, "optimizer": models.lstm_optimizers, "loss": models.loses},
