...
=========
    ...
    >>> import numpy as np
    ...
    >>> predictions = predictit.main.predict(np.random.randn(100, 1), predicts=3, plotit=1)
    ...
    if len(sys.argv) > 1:
        ...
        parser = argparse.ArgumentParser(
            usage=(
                ...
                'python main.py --predicts 30 --plot_name "My fancy plot"'
            ),
        )

        for i in Config.get_dict().keys():
            parser.add_argument(f"--{i}")

        # Non empty command line args
        parser_args_dict = {
            k: mypythontools.misc.infer_type(v)
...
def predict(positional_data=None, positional_predicted_column=None, **function_kwargs):
    ...
    mylogging.set_warnings(Config.debug, Config.ignored_warnings, Config.ignored_warnings_class_type)
    ...
    config_errors = set(Config.get_dict().keys()) - configuration.all_variables_set
    ...
    used_models_assigned = {
        i: j for (i, j) in predictit.models.models_assignment.items() if i in Config.used_models
    }
            ...
            for iterated_model_index, (
                ...
                if Config.models_input[iterated_model_name] == input_type_name:
                    ...
                    predict_parameters = {
                        ...
                        "ignored_warnings_class_type": Config.ignored_warnings_class_type,
                    ...
                    if Config.multiprocessing == "process":
                        ...
                        results = {
                            **results,
                            **predictit.main_loop.train_and_predict(**predict_parameters),
                        }
    ...
    results_to_drop = [
        i for i in ["Index", "Trained model", "Test errors", "Results"] if i in results_df.columns
    ]
    ...
    if predictions.empty:
        raise RuntimeError(
            mylogging.return_str(
                "Neither of models finished prediction. Check warnings (Config.debug = 1) for more info."
            )
        )
    ...
    if Config.mode == "validate":
        ...
        predictions_for_plot["Test"] = test
    ...
    if Config.printit:
        ...
        if Config.print_table == 1:
            print(
                f"\n{tabulate(small_results_table.values, headers=['Model', f'Average {Config.error_criterion} error'], **Config.table_settigs)} \n"
            )

        elif Config.print_table == 2:
            print(f"\n{tabulate(results_df.values, headers=results_df.columns, **Config.table_settigs)} \n")

        if Config.print_time_table:
            print(f"\n{tabulate(time_df.values, headers=time_df.columns, **Config.table_settigs)} \n")
...
def predict_multiple_columns(positional_data=None, positional_predicted_columns=None, **function_kwargs):
    ...
    Args:
        ...
        **kwargs (dict): There is much more parameters in this function. Check configuration.py or run predictit.configuration.print_config() for parameters details.
...
def compare_models(positional_data_all=None, positional_predicted_column=None, **function_kwargs):
    ...
    for i in all_data_average:
        ...
        optimization_params.append(
            Config.optimization_values[int(np.nanargmin(i))] if not np.isnan(i).all() else np.nan
        )
    ...
    print(f"\n{tabulate(models_table.values, headers=models_table.columns, **Config.table_settigs)} \n")
