...
def train_and_predict(
        ...
        data_abs_max, data_mean, data_std, last_undiff_value, models_test_outputs, final_scaler,
        pipe=None):
...
def predict(data=None, predicts=None, predicted_column=None, freq=None, models_parameters=None, data_source=None, csv_full_path=None, plot=None,
                ...
                if config['models_input'][iterated_model_name] == input_type:
                    ...
                    if config['multiprocessing'] == 'process':
                        ...
                        p = multiprocessing.Process(target=train_and_predict, args=(
                            iterated_model.train, iterated_model.predict, iterated_model_name, iterated_model_index, data_length_index,
                            data_length_iteration, data_lengths, model_train_input, model_predict_input, model_test_inputs, config['models_parameters'][iterated_model_name],
                        ...
                        pool.apply_async(train_and_predict, args=(
                            iterated_model.train, iterated_model.predict, iterated_model_name, iterated_model_index, data_length_index,
                            data_length_iteration, data_lengths, model_train_input, model_predict_input, model_test_inputs, config['models_parameters'][iterated_model_name],
                            config['predicts'], config['optimizeit'], config['data_transform'], config['standardize'], config['power_transformed'], config['repeatit'], config['error_criterion'],
                            data_abs_max, data_mean, data_std, last_undiff_value, models_test_outputs, final_scaler,
                        ...
                        results.append(train_and_predict(iterated_model.train, iterated_model.predict, iterated_model_name, iterated_model_index, data_length_index,
                            data_length_iteration, data_lengths, model_train_input, model_predict_input, model_test_inputs, config['models_parameters'][iterated_model_name],
                            config['predicts'], config['optimizeit'], config['data_transform'], config['standardize'], config['power_transformed'], config['repeatit'], config['error_criterion'],
                            data_abs_max, data_mean, data_std, last_undiff_value, models_test_outputs, final_scaler))
    ...
    if config['print']:
        ...
        models_table.field_names = ['Model', f"Average {config['error_criterion']} error"]  # , "Time"]  # TODO
        # Fill the table
        for i, j in predicted_models_for_table.items():
            models_table.add_row([i, j['error_criterion']])  # , big_dict[i]['model_time']])
        ...
        if config['print_detailed_result']:
            ...
            for i, j in enumerate(models_names):
                ...
                #     print(f"\t Time to optimize {big_dict[i]['optimization_time']} \n")
