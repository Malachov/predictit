...
def train_and_predict(
        ...
        optimization_value, option_optimization_list, model_train_input, model_predict_input, model_test_inputs, only_predict,
    ...
    try:
        ...
        process_repeated_matrix = np.zeros((repeatit, config['predicts']))
...
def predict(data=None, predicts=None, predicted_column=None, freq=None, models_parameters=None, data_source=None, csv_full_path=None, plot=None,
    ...
    for optimization_index, optimization_value in enumerate(option_optimization_list):
        ...
        if config['optimization_variable']:
            config[config['optimization_variable']] = optimization_value
        ...
        for input_type in used_input_types:
            try:
                ...
                traceback_warning(f"Error in creating sequentions on input type: {input_type} with option optimization: {optimization_value}")
                continue
            ...
            for iterated_model_index, (iterated_model_name, iterated_model) in enumerate(config['used_models'].items()):
                if config['models_input'][iterated_model_name] == input_type:
                    ...
                    only_predict = 1 if iterated_model_name in only_predict_dict else 0
                    ...
                    if isinstance(used_sequention, tuple):
                        model_train_input = (used_sequention[0][: data_end, :], used_sequention[1][: data_end, :])
                        model_predict_input = used_sequention[2]
                        model_test_inputs = used_sequention[3]

                    elif used_sequention.ndim == 1:
                        if only_predict:
                            model_train_input = model_predict_input = used_sequention




                        model_train_input = used_sequention[: data_end]
                        model_predict_input = used_sequention[:]
                        model_test_inputs = []
                        for i in range(config['repeatit']):
                            model_test_inputs.append(used_sequention[: - config['predicts'] - config['repeatit'] + i + 1])
                    ...
                    predict_parameters = {
                        ...
                        'model_test_inputs': model_test_inputs, 'only_predict': only_predict, 'data_abs_max': data_abs_max, 'data_mean': data_mean, 'data_std': data_std,
    ...
    if config['print']:
        if config['print_result']:
            ...
            # TODO
            # print((f"\n\t with data length {option_optimization_list[predicted_models_for_table[best_model_name]['data_length']]} \n\t with paramters "
        ...
        if config['print_detailed_result']:
            ...
            detailed_table.field_names = ['Name', f"Average {config['error_criterion']} error", 'Time', 'Config optimization']
            for i in results:
                try:
                    detailed_table.add_row([i['name'], round(i['model_error'], 3), round(i['model_time'], 3), i['optimization_value']])
                except Exception:
                    pass
