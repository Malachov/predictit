...
def train_and_predict(
    ...
    """Inner function, that can run parallel with multiprocessing.
    ...
    mylogging.set_warnings(debug, ignored_warnings, ignored_warnings_class_type)
    ...
    result_name = (
        f"{iterated_model_name} - {optimization_value}" if optimization else f"{iterated_model_name}"
    )
    ...
    try:
        ...
        for repeat_iteration in range(repeatit):
            ...
            tests_results[repeat_iteration] = iterated_model_predict(
                model_test_inputs[repeat_iteration], trained_model, predicts=predicts
            )
        ...
        mylogging.traceback(
            caption=f"Error in '{result_name}' model"
            if not optimization
