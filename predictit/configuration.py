...
class Config:
    ...
    predicted_columns = (
        []
    )  # For predict_multiple function only! List of names of predicted columns or it's indexes. If 'data' is dataframe or numpy, then you can use ['*'] to predict all the columns with numbers.
    ...
    optimization_values = [
        ...
        8,
        12,
    ]  # List of evaluatedconfig values. Results of each value are only in detailed table.
    ...
    analyze_seasonal_decompose = {
        "period": 365,
        "model": "additive",
    }  # Parameters for seasonal decompose in analyze. Find if there are periodically repeating patterns in data.
    ...
    def update_references_input_types(cls):
        ...
        cls.input_types = {
            ...
            "one_step_constant": {
                "n_steps_in": cls.default_n_steps_in,
                "n_steps_out": 1,
                "default_other_columns_length": cls.default_other_columns_length,
                "constant": 1,
            },
            "one_step": {
                "n_steps_in": cls.default_n_steps_in,
    ...
    models_input = {
        ...
        **{
            ...
            for model_name in ["Regression", "Levenberg-Marquardt", "Ridge regression"]
        },
        **{
    ...
    def update_references_optimize(cls):
        cls.models_parameters_limits = {
            ...
            "Bayes ridge regression": {
                "alpha_1": [0.1e-6, 3e-6],
                "alpha_2": [0.1e-6, 3e-6],
                "lambda_1": [0.1e-6, 3e-6],
                "lambda_2": [0.1e-7, 3e-6],
            },
    ...
    with open(Path(__file__).resolve(), "r") as f:
        print(
            pygments.highlight(
                ...
                pygments.formatters.Terminal256Formatter(style="friendly"),
            )
        )
